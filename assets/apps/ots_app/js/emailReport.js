...
// TODO: docelowo link do raportu w Fiege Cloud — na razie placeholder, do podmiany
// gdy będzie znany docelowy adres.
const REPORT_LINK_PLACEHOLDER = 'OTS PowerBI Report';

const TABLE_STYLE = 'border-collapse:collapse;font-family:Calibri,Arial,sans-serif;font-size:11pt;margin:4px 0 12px;';
const TH_STYLE = 'border:1px solid #999999;padding:4px 10px;background:#f2f2f2;text-align:left;';
...
function htmlTable(headers, rows) {
  ...
  const tbody = rows
    .map((row) => `<tr>${row.map((cell) => `<td style="${TD_STYLE}">${escapeHtml(cell)}</td>`).join('')}</tr>`)
    .join('');
...
function explainedRows(dayLines, reviewsByObd) {
  return groupNeedingReviewByObd(dayLines)
    .map((g) => ({ country: g.country || '—', wmsOrder: g.wmsOrder || '—', review: reviewsByObd[g.obd] }))
    .filter((g) => g.review && g.review.reasonCode)
    .map(({ country, wmsOrder, review }) => [country, wmsOrder, review.reasonCode]);
}

export function buildEmailReport({ config, enrichedLines, selectedDate, reviewsByObd }) {
  const dayLines = filterByAdjustedDate(enrichedLines, selectedDate);
  const monthLines = filterByAdjustedMonth(enrichedLines, selectedDate);
  ...
  const countryHeaders = ['Country', 'Total', 'On Time', 'Missed', 'J CONFIRMATION', '% OTS'];
  const countryRowsFormatted = countryRows.map((row) => [
    row.country, row.total, row.onTime, row.missed, row.jConfirmation, pct(row.grossPct),
  ]);
  ...
  textLines.push('');
  textLines.push('Pełne dane możesz obejrzeć pod adresem:');
  textLines.push(REPORT_LINK_PLACEHOLDER);

  // --- Wersja HTML (prawdziwe <table> — to ona daje sformatowaną tabelę po wklejeniu w Outlooku) ---
  const explainedTable = explained.length === 0
  ...
  const htmlBody = `
    <div style="font-family:Calibri,Arial,sans-serif;font-size:11pt;">
      ...
      <p>Pełne dane możesz obejrzeć pod adresem:<br>${escapeHtml(REPORT_LINK_PLACEHOLDER)}</p>
    </div>
  `.trim();
